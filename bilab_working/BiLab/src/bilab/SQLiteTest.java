...
public class SQLiteTest implements SQLite.Callback, SQLite.Function, SQLite.Authorizer, SQLite.Trace, SQLite.ProgressHandler {
  ...
  public static void test() {
    ...
    try {
      db.open("U:\\dev\\GTL\\win\\BiLabRCP\\BiLab\\resources\\test.db", 0666);
      System.out.println("DB version: " + db.dbversion());
      db.interrupt();
      db.busy_timeout(1000);
      db.busy_handler(null);
      db.exec("create table TEST (firstname char(32), lastname char(32) )", new SQLiteTest()); //-DJ
      ...
      db.exec("select myregfunc(TEST.firstname) from TEST", new SQLiteTest());
      System.out.println("==== call aggregate function ====");
      db.exec("select myaggfunc(TEST.firstname) from TEST", new SQLiteTest());
