/*
 * Jalview - a java multiple alignment editor Copyright (C) 1998 Michele Clamp
 * 
...
public class AlignApplet extends Applet {
    ...
    public void makeFrame() {
        try {
            final String s = getParameter("numseqs");
            if (getParameter("numseqs") == null) {
                af = new AlignFrame(this, input, type, format);
            } else {
                final int num = (Integer.valueOf(getParameter("numseqs")))
                        .intValue();
                int i = 0;
                int count = 0;
                final DrawableSequence[] seqs = new DrawableSequence[num];
                while (i < num) {
                    final String s2 = getParameter("seq" + (i + 1));
                    if (s2 != null) {
                        String id = getParameter("id" + (i + 1));
                        if (id == null) {
                            id = "Seq_" + i;
                        }
                        seqs[count] = new DrawableSequence(id, s2, 1,
                                s2.length());
                        count++;
                    } else {
            ...
            if (srsServer != null) {
                if (!(srsServer.equals(""))) {
                    ...
                    if (!srsServer.substring(srsServer.length() - 1)
                            .equals("/")) {
                        srsServer = srsServer + "/";
                    }
            ...
            try {
                final int fs = (Integer.valueOf(fontSize)).intValue();
                // af.ap.seqPanel.seqCanvas.f = new
                // Font("Courier",Font.PLAIN,fs);
            ...
            if (consString.equals("*")) {
                ...
                final int seqs[] = selectSeqs(consString);
                int i = 0;
                af.ap.selectAll(false);
                while (i < seqs.length && seqs[i] != -1) {
                    af.ap.sel
                            .addElement(af.ap.seqPanel.align.sequences[seqs[i]]);
                    i++;
                }
            }
            ...
            if (noGroups > 0) {
                for (int i = 0; i < noGroups; i++) {
                    ...
                    final String gs = getParameter("group" + (i + 1));
                ...
                // PIDColourScheme pidcs = new
                // PIDColourScheme(af.ap.seqPanel.align.cons);
                final ClustalxColourScheme pidcs = new ClustalxColourScheme(
                        af.ap.seqPanel.align.cons2, af.ap.seqPanel.align.size());
    ...
    public void parseGroup(final Alignment al, final String gs) {
        int col = 0;
        boolean boxes = true;
        boolean text = true;
        boolean colourText = false;

        final StringTokenizer st = new StringTokenizer(gs, ":");
        try {
            final int seqs[] = selectSeqs(st.nextToken());
            ...
            final SequenceGroup sg = new SequenceGroup(cs, true, boxes, text,
                    colourText, true);
            ...
            while (seqs[i] != -1) {
                if (af.ap.seqPanel.align.sequences[seqs[i]] != null) {
                    if (af.ap.seqPanel.align.findGroup(seqs[i]) != null) {
                        af.ap.seqPanel.align.removeFromGroup(
                                af.ap.seqPanel.align.findGroup(seqs[i]),
    ...
    public int[] selectSeqs(final String s) {
        final StringTokenizer st = new StringTokenizer(s, ",");

        final int[] seqs = new int[2000];
        int count = 0;
        ...
        while (st.hasMoreTokens()) {
            ...
            if (tmp.equals("*")) {
                ...
                try {
                    final StringTokenizer st2 = new StringTokenizer(tmp, "-");

                    final int start = (Integer.valueOf(st2.nextToken()))
                            .intValue();
                    final int end = (Integer.valueOf(st2.nextToken()))
                            .intValue();
