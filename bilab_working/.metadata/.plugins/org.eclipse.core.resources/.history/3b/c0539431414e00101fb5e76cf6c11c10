...
public abstract class Graphic extends ModelElement {
  ...
  {
    try {
      InputStream stream = BilabPlugin.findResourceStream(name);
      Image image = new Image(null, stream);
      try { stream.close(); } catch (IOException e) {} // ignore close errors
      return image;
    } catch (IOException ioe) {
      bilab.Notify.devWarning(Graphic.class, "resource not found:"+name);
      return null;
    }
  }
